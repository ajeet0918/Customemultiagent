...
(async () => {
  ...
  try { let code=await run('exercise'); if (!code) code=await run('persistence'); if (!code && (calls!==7 || catalogCalls!==1)) throw new Error(`Expected seven completions and one catalog call; got ${calls} completions and ${catalogCalls} catalogs`); if(code) process.exitCode=code; else console.log('Desktop integration passed: settings, themes, standalone models, projectless chat, MCP discovery, approved/declined tools, project writes, persistence and sandbox isolation.'); }
